  ...
  const startPolling = async (orderId: string) => {
    const pollInterval = setInterval(async () => {
      try {
        ...
        console.log(`Polling attempt ${pollCount + 1}/${maxPollAttempts} for order ${orderId}`);

        // Gọi backend để kiểm tra trạng thái thanh toán
        const response = await api.get(`/payment/zalopay/status/${orderId}`);
        ...
        if (response.paymentStatus === 'PAID') {
          ...
          setTimeout(() => {
            router.push(`/payment/success?orderId=${orderId}&verified=true`);
          }, 2000);
          
        } else if (response.paymentStatus === 'FAILED' || response.paymentStatus === 'CANCELLED') {
          // Thanh toán thất bại
          setVerificationStatus('failed');
          setErrorMessage('Thanh toán thất bại hoặc bị hủy');
          toast.error("Thanh toán thất bại");
          clearInterval(pollInterval);
          
        } else if (pollCount >= maxPollAttempts - 1) {
          // Hết thời gian polling
          setVerificationStatus('failed');
          setErrorMessage('Hết thời gian chờ xác thực thanh toán. Vui lòng kiểm tra lại sau.');
          toast.error("Hết thời gian chờ xác thực");
          clearInterval(pollInterval);
        }
